log_file_full = load('./data/EKF_stm32_test2_r.txt');
starting_row =1;
end_row = 1000;
...
%   size(mag)
 ...
 y1 = sosfilt(SOS,acc(:,1))*0.0003
 y2 = sosfilt(SOS,acc(:,2))*0.0003
 y3 = sosfilt(SOS,acc(:,3))*0.0003
...
%      [x,P] = IMU_EKF(x, t, P, Q, R_full,acc(k,1),acc(k,2), acc(k,3),...
%                                             gyro(k, 1), gyro(k, 2), gyro(k, 3)); 
             [x, P] = IMU_EKF(x, t, P, Q, R_full, y1(k),y2(k), y3(k),...
                                           gyro(k, 1), gyro(k, 2), gyro(k, 3)); 
